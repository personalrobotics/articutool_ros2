    ...
    def draw_frame(self):
        ...
        try:
            rotated_axes = np.array(
                [
                    quaternions.rotate_vector(axis, self.quaternion)
                    for axis in frame_axes
                ]
            )
        except Exception as e:
            self.get_logger().error(
                f"Error rotating vector: {e}. Quaternion: {self.quaternion}"
            )
        ...
        self.ax.quiver(
            ...
            rotated_axes[0, 0],
            rotated_axes[0, 1],
            rotated_axes[0, 2],  # Direction vector (rotated X)
            color="r",
            length=0.5,
        ...
        self.ax.quiver(
            ...
            rotated_axes[1, 0],
            rotated_axes[1, 1],
            rotated_axes[1, 2],  # Direction vector (rotated Y)
            color="g",
            length=0.5,
            arrow_length_ratio=0.1,
        )
        ...
        self.ax.quiver(
            ...
            rotated_axes[2, 0],
            rotated_axes[2, 1],
            rotated_axes[2, 2],  # Direction vector (rotated Z)
            color="b",
            length=0.5,
            arrow_length_ratio=0.1,
        )
        ...
        self.ax.text(
            rotated_axes[0, 0] * 0.55,
            rotated_axes[0, 1] * 0.55,
            rotated_axes[0, 2] * 0.55,
            "X",
            color="r",
        )
        self.ax.text(
            rotated_axes[1, 0] * 0.55,
            rotated_axes[1, 1] * 0.55,
            rotated_axes[1, 2] * 0.55,
            "Y",
            color="g",
        )
        self.ax.text(
            rotated_axes[2, 0] * 0.55,
            rotated_axes[2, 1] * 0.55,
            rotated_axes[2, 2] * 0.55,
            "Z",
            color="b",
        )
