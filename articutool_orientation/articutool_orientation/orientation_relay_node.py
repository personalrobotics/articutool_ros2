...
class OrientationRelay(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "input_topic", "/articutool/imu_data_and_orientation_calibrated"
        )
        ...
        self.publisher_ = self.create_publisher(QuaternionStamped, output_topic, 10)
        ...
        self.get_logger().info(
            f"Relaying 'orientation' field from '{input_topic}' [{Imu.__name__}]"
        )
        self.get_logger().info(
            f"Publishing to '{output_topic}' [{QuaternionStamped.__name__}]"
        )
        ...
        try:
            if not hasattr(msg, "header"):
                self.get_logger().warn(
                    f"Input message on topic '{self.subscription.topic_name}' missing 'header' field. Skipping.",
                    throttle_duration_sec=5,
                )
                return
            if not hasattr(msg, "orientation"):
                self.get_logger().warn(
                    f"Input message on topic '{self.subscription.topic_name}' missing 'orientation' field. Skipping.",
                    throttle_duration_sec=5,
                )
                return
            if not isinstance(msg.orientation, Quaternion):
                self.get_logger().warn(
                    f"Field 'orientation' in input message is not a Quaternion. Type: {type(msg.orientation)}. Skipping.",
                    throttle_duration_sec=5,
                )
                return
